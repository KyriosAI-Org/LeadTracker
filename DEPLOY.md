...
em runtime via `env_file: .env`.

## Passo a passo (Portainer)

1. **DNS**: crie um registro `A` de `leadtracker.kyriosai.me` apontando para o IP da VPS.
...
3. **Código + segredos na VPS**: clone o repositório e crie o `.env` a partir do
   ...
   ```bash
   cp .env.example .env && nano .env   # preencha as chaves do Supabase
   ```
4. **Stack no Portainer**:
   ...
   - Em **Environment variables** do Portainer, replique as `VITE_*` (elas
     alimentam os `build args`).
5. **Deploy**. O Traefik detecta as labels e emite o certificado automaticamente.
   Acesse `https://leadtracker.kyriosai.me`.
